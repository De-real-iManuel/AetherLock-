...
export const DisputeForm: React.FC<DisputeFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload files to IPFS (optional evidence)
      const evidence = files.length > 0 ? await uploadFiles() : [];
  ...
  return (
    ...
    >
      {/* Warning Banner */}
      <div className="mb-6 p-4 rounded-lg bg-status-failed/10 border border-status-failed/30">
        <div className="flex items-start gap-3">
          <AlertTriangle className="w-5 h-5 text-status-failed flex-shrink-0 mt-0.5" />
          <div>
            <h3 className="text-sm font-semibold text-status-failed mb-1">
              Opening a Dispute
            </h3>
            <p className="text-xs text-slate-300">
              Disputes should only be opened if the submitted work does not meet the agreed requirements. 
              Please provide a detailed explanation and supporting evidence. This action will be reviewed 
              and may affect trust scores.
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Explain in detail why you are disputing this work. Include specific issues, unmet requirements, or quality concerns..."
            className={cn(
              "w-full min-h-[150px] rounded-md border bg-primary-surface/50 backdrop-blur-sm px-4 py-3 text-sm text-white placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-electric focus-visible:border-accent-electric transition-all duration-200 resize-y",
              "border-primary-border hover:border-accent-electric/50"
            )}
            disabled={isSubmitting}
          />
          <p className="mt-1 text-xs text-slate-400">
            Minimum 20 characters • {reason.length} characters
          </p>
        </div>
